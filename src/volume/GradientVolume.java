        ...
        for (int i=1; i<volume.getDimX()-1;i++) {
            for (int j=1; j<volume.getDimY()-1;j++) {
                for (int k=1; k<volume.getDimZ()-1;k++) {
                    ...
                    float dx = ((volume.getVoxel(i-1, j, k) - volume.getVoxel(i+1, j, k)) / 2.0f);
                    float dy = ((volume.getVoxel(i, j-1, k) - volume.getVoxel(i, j+1, k)) / 2.0f);
                    float dz = ((volume.getVoxel(i, j, k-1) - volume.getVoxel(i, j, k+1)) / 2.0f);
